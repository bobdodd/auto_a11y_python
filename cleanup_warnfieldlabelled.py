#!/usr/bin/env python3
"""
Cleanup script to remove WarnFieldLabelledByElementThatIsNotALabel entries from database.
This warning code was never fully implemented and is redundant with ErrFielLabelledBySomethingNotALabel.
"""
...
query = {'expected_code': 'WarnFieldLabelledByElementThatIsNotALabel'}
results = list(fixture_tests_collection.find(query))

print(f"Found {len(results)} database entries for WarnFieldLabelledByElementThatIsNotALabel")
...
if results:
    ...
    if remaining == 0:
        print("\n✓ Successfully removed all WarnFieldLabelledByElementThatIsNotALabel entries from database")
    else:
