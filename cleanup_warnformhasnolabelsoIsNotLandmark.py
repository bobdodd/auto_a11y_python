#!/usr/bin/env python3
"""
Cleanup script to remove WarnFormHasNoLabelSoIsNotLandmark entries from database.
This warning was a duplicate of WarnFormHasNoLabel (both already removed).
"""
...
query = {'expected_code': 'WarnFormHasNoLabelSoIsNotLandmark'}
results = list(fixture_tests_collection.find(query))

print(f"Found {len(results)} database entries for WarnFormHasNoLabelSoIsNotLandmark")
...
if results:
    ...
    if remaining == 0:
        print("\n✓ Successfully removed all WarnFormHasNoLabelSoIsNotLandmark entries from database")
    else:
