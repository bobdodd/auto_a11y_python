#!/usr/bin/env python3
"""
Script to add ALL French translations for the reports/dashboard.html page
"""
...
# Dictionary of English to French translations - COMPLETE list
translations = {
...
for english, french in translations.items():
    ...
    if entry:
        if not entry.msgstr or entry.msgstr == "":
            ...
            translated_count += 1
            print(f"✓ Added translation: {english[:60]}...")
        else:
            print(f"→ Already translated: {english[:60]}...")
    else:
        not_found_count += 1
        print(f"✗ Not found in catalog: {english[:60]}...")

# Save the updated .po file
po.save('auto_a11y/web/translations/fr/LC_MESSAGES/messages.po')
print(f"\n✓ Translations saved!")
print(f"  - Added {translated_count} new translations")
print(f"  - {not_found_count} strings not found in catalog (will be added after next extract)")
