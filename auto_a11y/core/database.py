...
class Database:
    ...
    def get_latest_test_results_per_user(self, page_id: str) -> List[TestResult]:
        """
        Get the most recent test result for each unique user who tested this page.

        Returns a list of TestResult objects, one for each user (including guest).
        Results are sorted by test_date descending.
        """
        # Use MongoDB aggregation to group by user and get latest per user
        pipeline = [
            {"$match": {"page_id": page_id}},
            {"$sort": {"test_date": -1}},
            {"$group": {
                "_id": {
                    "$ifNull": ["$metadata.authenticated_user.user_id", "guest"]
                },
        ...
        docs = list(self.test_results.aggregate(pipeline))
        results = []
