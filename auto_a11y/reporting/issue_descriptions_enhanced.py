...
def get_detailed_issue_description(issue_code: str, metadata: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    descriptions = {        'AI_ErrAccordionWithoutARIA': {
        ...
        'ErrTabindexChildOfInteractive': {
            'title': "Child element with tabindex inside interactive parent creates redundant focus and inaccessible focus indicator for screen magnifier users",
            'what': "Child element (such as SVG, span, icon, or other nested element) has a tabindex attribute making it independently focusable when it is already contained within an interactive parent element like a button, link (anchor), or other focusable control",
            'why': "When a child element inside a button or link has its own tabindex, it creates multiple severe accessibility issues. First, it creates a redundant tab stop - users must tab twice to get past what appears to be a single button. Second and most critically, the child element typically relies on its parent's focus indicator rather than having its own. Screen magnifier users at 200-400% zoom often see only a small portion of the page - they may see only the SVG icon without seeing the surrounding button border or background. When the parent button changes background color on focus, magnified users viewing only the child SVG see no change at all and cannot tell the element has focus. Even if the parent has an outline, if the magnified viewport shows only the child element, the user sees no focus indicator whatsoever. This makes it impossible for screen magnifier users to track focus position when interacting with buttons containing icons, SVGs, or text spans. The child element would need its own distinct, visible focus indicator to be accessible at high zoom, but this is almost never implemented and creates visual redundancy.",
            'who': "Screen magnifier users at 200%+ zoom (required by WCAG 1.4.4) who may only see the child element without parent context, users with tunnel vision or reduced peripheral vision who cannot see beyond the focused element, users at 400% magnification (accommodated by WCAG 1.4.10) where only a small screen region is visible, users with low vision navigating with high zoom, keyboard users confused by redundant tab stops on what appears to be a single interactive element, users with cognitive disabilities who may be confused by nested focusable elements, and users who rely on focus indicators to track position but cannot see the parent element's indicator",
            'impact': ImpactScale.HIGH.value,
            'wcag': ['2.4.7', '1.4.4', '1.4.10'],
            'remediation': "Remove the tabindex attribute from the child element - the parent interactive element already provides complete keyboard access. For example, if you have <button><svg tabindex=\"0\">...</svg></button>, change it to <button><svg>...</svg></button>. The button element is already focusable and clickable, so the SVG child does not need tabindex. This fixes both the redundant tab stop issue and the screen magnifier accessibility issue. In 99% of cases, this is the correct solution. Only keep tabindex on child elements if there is a specific, documented reason for the child to be independently focusable separate from the parent (which is extremely rare). If you must keep the child independently focusable (rare edge cases like complex custom widgets), you must add a distinct, visible focus indicator directly to the child element that is perceivable when viewing only that element in isolation: svg[tabindex]:focus { outline: 2px solid #0066cc; outline-offset: 2px; }. However, removing tabindex from children is almost always better - it simplifies keyboard navigation, eliminates confusion, and relies on the native interactive element's built-in accessibility."
        },
