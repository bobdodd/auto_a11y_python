...
Translates Phase 1 accessibility issue descriptions to French using:
- W3C WCAG 2.2 official French terminology
- Context-aware translation for accessibility terms
...
# W3C WCAG terminology for reference
WCAG_TERMS = {
...
Example:
[
  {{"index": 0, "translation": "Image sans attribut alt"}},
  {{"index": 1, "translation": "Les utilisateurs de lecteurs d'écran..."}}
]
...
def translate_batch(client: anthropic.Anthropic, entries: List[Dict], category: str) -> List[Dict]:
    """Translate a batch of entries using Claude API"""
...
def main():
    # Check for API key (try both ANTHROPIC_API_KEY and CLAUDE_API_KEY)
    ...
    # Process by category in batches
    batch_size = 75
    all_translated = []
    ...
    total_batches = sum((len(entries) + batch_size - 1) // batch_size for entries in by_category.values())
    batch_num = 0

    for category, cat_entries in by_category.items():
        print(f"\n{category}: {len(cat_entries)} strings")
        ...
        for i in range(0, len(cat_entries), batch_size):
            ...
            translated = translate_batch(client, batch, category)
    ...
    # Save results
    output_file = 'phase1_translated.json'
    output_data = {
        'metadata': {
