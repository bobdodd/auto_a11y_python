#!/usr/bin/env python3
"""
Cleanup script to remove WarnFormHasNoLabel entries from database.
This warning was redundant with WarnFormHasNoLabelSoIsNotLandmark.
"""
...
query = {'expected_code': 'WarnFormHasNoLabel'}
results = list(fixture_tests_collection.find(query))

print(f"Found {len(results)} database entries for WarnFormHasNoLabel")
...
if results:
    ...
    if remaining == 0:
        print("\n✓ Successfully removed all WarnFormHasNoLabel entries from database")
    else:
